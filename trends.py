...
def analyze_tweet_sentiment(tweet):
    ...
    for elem in words:
        if has_sentiment(get_word_sentiment(elem)):
            lst.append(sentiment_value(get_word_sentiment(elem)))
        else:
            pass
    ...
    if len(lst) == 0:
        ...
        return make_sentiment(sum(lst)/len(lst))
...
def find_centroid(polygon):
    ...
    for i in range(len(polygon) - 1):
        x1, x2 = latitude(polygon[i]), latitude(polygon[i+1])
        y1, y2 = longitude(polygon[i]), longitude(polygon[i+1])
        sub_area = x1 * y2 - x2 * y1
        area += sub_area
        centerX += (x1 + x2) * sub_area
        centerY += (y1+y2)* sub_area
            
    area = area *(1/2)
            
    
    if area == 0:
        centerX = latitude(polygon[0])
        centerY = longitude(polygon[0])
        area = 0
    ...
    return [centerX, centerY, abs(area)]
...
def find_state_center(polygons):
    ...
    areaXY = 0
    for ply in polygons:
        Cx, Cy, poly_area = find_centroid(ply);
        centerX += Cx* poly_area
        centerY += Cy * poly_area
        areaXY += poly_area
        
    
    centerX = centerX/areaXY
    centerY = centerY/areaXY
    return make_position(centerX, centerY)
...
def group_tweets_by_state(tweets):
    ...
    tweets_by_state = {}
    "*** YOUR CODE HERE ***"
    state_list = [i for i in us_states.keys()]
    
    def find_closest_state(twt_loc):
        closest_state = 'CA'
        stat_dis = geo_distance(twt_loc, find_state_center(us_states[closest_state]))
        for i in state_list:
            state_cent = find_state_center(us_states[i])
            state_twt_loc = geo_distance(twt_loc,state_cent)
            if stat_dis > state_twt_loc:
                stat_dis = state_twt_loc
                closest_state = i
        return closest_state
    
    for w in tweets:
        state = find_closest_state(tweet_location(w))
        if state in tweets_by_state:
            tweets_by_state[state].append(w)
        else:
            tweets_by_state[state] = [w]
    return tweets_by_state
      
    """
        state_list += [i for i in us_states.keys()]
    min_dist = geo_distance(tweet_location(temp_tweet),find_state_center(us_states['CA']))
    
    for i in range(len(tweets)):
        temp_tweet = tweets[i]
        
        for j in range(len(state_list)):
            dist = geo_distance(tweet_location(temp_tweet),find_state_center(us_states[state_list[j]]))
            
            if dist < min_dist:
                min_dist = dist
                closest_state = state_list[j]
                
        tweets_by_state[closest_state].append(temp_tweet)
...
def average_sentiments(tweets_by_state):
    ...
    for j in tweets_by_state.keys():
            
        sent_value = 0
        sent_final = 0
        sentiment_list = []
        for single_tweet in tweets_by_state[j]:
            
            if has_sentiment(analyze_tweet_sentiment((single_tweet))) == False:
                pass  
            else:
                sent_value = sentiment_value(analyze_tweet_sentiment(single_tweet))
                sentiment_list.append(sent_value)
        
        if len(sentiment_list) == 0:
            pass
        else: 
            sent_final = sum(sentiment_list)/len(sentiment_list)
            averaged_state_sentiments[j] = sent_final
